...
async function obtenerLead(telefono) {
  return await redis.get(leadKey(telefono));
}

async function buscarLeadPorNombre(nombre) {
...
async function listarTodosLeads() {
  const keys = await redis.keys(`${LEAD_KEY_PREFIX}*`);
...
async function actualizarLead(telefono, cambios, fuenteUpdate, textoUpdate) {
...
function calcularUrgencia(lead) {
  ...
  if (proxima && proxima < ahora) {
    const dias = Math.floor((ahora - proxima) / (1000 * 60 * 60 * 24));
...
function calcularScoreFinal(lead) {
...
async function listarLeadsPrioritarios(limite = 10) {
...
function parsearEventoTecnom(evento) {
  ...
  return { nombre, telefono, email, accion_realizada: accionRealizada, proxima_accion: proximaAccion, tipo_gestion: tipoGestion, fecha_evento: fechaEvento, evento_id: evento.id };
}

async function sincronizarLeadsConCalendar(eventos) {
  const ahora = new Date();
