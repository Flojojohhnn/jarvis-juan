...
// ============================================================
// Tools expuestas al bot (8 tools)
// ============================================================
...
export const TOOLS = [
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        desde: { type: 'string', description: 'ISO date o datetime, ej: 2026-04-23' },
        hasta: { type: 'string', description: 'ISO date o datetime, ej: 2026-04-30' },
      },
  ...
  },
  {
    ...
    description: 'Lee los eventos del calendar y actualiza los leads cargados en Jarvis.',
    input_schema: { type: 'object', properties: {} },
  },
];
...
const SYSTEM_CHAT = `Sos Jarvis, asistente personal de Juan (asesor Plan Óvalo en Ford Goldstein Mendoza).
...
export async function extraerCamposDeResumen(resumen) {
  ...
  return parsearJSON(texto, {
    ...
    objecion_principal: null, proxima_accion_sugerida: null,
  });
}
...
export async function transcribirAudio(base64, mediaType) {
  ...
  const extension = mediaType === 'audio/mpeg' ? 'mp3' : mediaType === 'audio/mp4' ? 'm4a' : mediaType === 'audio/wav' ? 'wav' : 'oga';
  const file = await toFile(buffer, `audio.${extension}`, { type: mediaType });
